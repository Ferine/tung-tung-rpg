...
It is pre-rendered now -- render_title.py paints it and gen_render.py bakes
it -- and this module cuts the baked picture into characters. build() below
is the painted title that came before, kept because the render takes its
letterforms from it and as the fallback if the bake has no title. What
follows describes that painting; the render keeps both of its effects and
does them with whole palette steps instead of rotated entries.
...
def cut(px, slots=None):
    ...
    tilemap entry has the flip bits anyway. With `slots` (the rendered title)
    a cell is only the same as another in the same palette."""
    pool = {}
    order = []
    entries = []
    for ty in range(H):
        for tx in range(W):
            cell = tuple(tuple(px[ty * 8 + y][tx * 8 + x] for x in range(8))
                         for y in range(8))
            pal = SLOTS[slots[ty][tx]] if slots else 2
            ...
            for key, fh, fv in ((cell, 0, 0), (hf, 1, 0),
                                (vf, 0, 1), (hv, 1, 1)):
                if (key, pal) in pool:
                    entries.append((pool[key, pal], pal, fh, fv))
                    break
            else:
                idx = len(order)
                pool[cell, pal] = idx
                order.append(cell)
                entries.append((idx, pal, 0, 0))
    return order, entries


# The rendered title spreads over BG palettes 2 and 4-7, like a backdrop, and
# its .pal holds one 2-7 block per animation step (render_title.py). It gets
# 512 characters: on the title BG1 borrows the field's window at $3000, which
# runs to the text map at $5000, and every way off the title loads a region
# into it again.
SLOTS = (2, 4, 5, 6, 7)
PAGE = 512
STEP = 5                        # video frames per palette step


def pal_block(pals):
    """{slot: 16 colours} -> slots 2-7, the way the CGRAM holds them."""
    return b''.join(g.palette_bin(pals.get(n, [(0, 0, 0)] * 16))
                    for n in range(2, 8))


def generate_title():
    from gen_sprites import load_renders
    baked = load_renders().get('title')
    if baked:
        px, slots = baked['idx'], baked['slot']
        frames = [{SLOTS[j]: [(0, 0, 0)] + p for j, p in enumerate(f)}
                  for f in baked['frames']]
    else:
        # No render: the painted title, still, one palette step.
        px, slots, frames = build().px, None, [{2: PAL}]
    tiles, entries = cut(px, slots)
    if len(tiles) > PAGE:
        raise SystemExit("title needs %d characters, BG1's window holds %d"
                         % (len(tiles), PAGE))
    ...
    words = [g.map_entry(idx, pal=pal, hflip=fh, vflip=fv)
             for idx, pal, fh, fv in entries]

    total = 0
    total += g.write('title.pic', sheet.to_pic())
    total += g.write('title.map', g.map_bin(words))
    total += g.write('title.pal', b''.join(pal_block(f) for f in frames))
    with open('src/titlemap.h', 'w') as f:
        ...
        f.write("#define TITLE_FRAMES %d\n#define TITLE_STEP %d\n"
                "#define TITLE_PAL_BYTES %d\n#define TITLE_PIC_BYTES %d\n"
                "\n#endif\n" % (len(frames), STEP, 6 * 32, PAGE * 32))
    print("title: %d characters, %d palette steps, %d bytes"
          % (len(tiles), len(frames), total))
    return len(tiles)
