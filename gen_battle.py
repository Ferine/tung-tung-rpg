...
Rendered backdrops come from render_backdrops.py via the bake in
assets/renders.txt; build() below paints any region that has none.
...
# A rendered backdrop spreads over five BG palettes (render_backdrops.py says
# why these five). The .pal file holds slots 2 to 7 as the CGRAM does, so the
# upload is two straight copies; slot 3 is the windows' and is never copied.
SLOTS = (2, 4, 5, 6, 7)
PAL_FIRST, PAL_LAST = 2, 7


def pal_file(pals):
    """pals: {slot: 16 colours}. Unused slots are written black."""
    return b''.join(g.palette_bin(pals.get(n, [(0, 0, 0)] * 16))
                    for n in range(PAL_FIRST, PAL_LAST + 1))


def cut(px, name, slots=None):
    """slots: the palette of each character (32x32), or None for all 2."""
    index = {}
    tiles = []
    entries = []
    for ty in range(ROWS):
        for tx in range(COLS):
            ...
            key = (tuple(tuple(r) for r in cell), pal)
            if key not in index:
                index[key] = len(tiles)
                tiles.append(cell)
            entries.append(g.map_entry(index[key], pal=pal, prio=0))
    if len(tiles) > 256:
...
def generate_battle():
    # Pre-rendered backdrops, baked by gen_render.py; a region without one
    # falls back to the painted builder below.
    from gen_sprites import load_renders
    renders = load_renders()
    lines = []
    dawn = {2: DAWN}
    for r in REGIONS:
        baked = renders.get('backdrop:' + r['key'])
        if baked:
            px, slots = baked['idx'], baked['slot']
            pals = {SLOTS[j]: [(0, 0, 0)] + p for j, p in
                    enumerate(baked['pals'])}
        else:
            px = build(r['horizon'], r['stars'], r['disc'], r['ridges'],
                       r['floor'], r['ceiling'])
            slots, pals = None, {2: r['pal']}
        sheet, entries, n = cut(px, r['key'], slots)
        g.write('bg_%s.pic' % r['key'], sheet.to_pic())
        g.write('bg_%s.map' % r['key'], g.map_bin(entries))
        g.write('bg_%s.pal' % r['key'], pal_file(pals))
        lines.append((r['key'], n))
        # The epilogue reuses the first backdrop's characters under a dawn
        # palette: the render's own dawn if it has one.
        if r is REGIONS[0] and baked and baked['dawn']:
            dawn = {SLOTS[j]: [(0, 0, 0)] + p for j, p in
                    enumerate(baked['dawn'])}

    g.write('bg_dawn.pal', pal_file(dawn))
    ...
    with open('bgdata.asm', 'w') as f:
        ...
        for r in REGIONS:
            ...
            for part in ('pic', 'map', 'pal'):
                f.write('bg_%s_%s: .incbin "%s"\n'
                        % (k, part, g.asset('bg_%s.%s' % (k, part))))
            f.write('.ends\n\n')
        f.write('.section ".rodata_bg_dawn" superfree\n'
                'bg_dawn_pal: .incbin "%s"\n.ends\n' % g.asset('bg_dawn.pal'))
